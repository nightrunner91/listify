...
export default async function importRoutes(app) {
  ...
  app.post('/import', {
    ...
    const existingCustomLists = await db.select({
      id: customLists.id,
      name: customLists.name 
    }).from(customLists).where(eq(customLists.userId, userId))
    const existingCustomListRecords = await db.select({
      id: customListRecords.id,
      listId: customListRecords.listId,
      title: customListRecords.title 
    }).from(customListRecords).where(eq(customListRecords.userId, userId))
